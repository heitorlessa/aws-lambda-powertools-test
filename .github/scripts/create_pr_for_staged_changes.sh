...
function create_pr() {
    ...
    export NEW_PR_URL=$(gh pr create --title "${PR_TITLE}" --body "${PR_BODY}: ${WORKFLOW_URL}" --base "${BRANCH}") # https://github.com/awslabs/aws-lambda-powertools/pull/13
    export NEW_PR_ID="${NEW_PR_URL##*/}"                                                                            # 13
}

function close_duplicate_prs() {
    debug "Do we have any duplicate PRs?"
    export DUPLICATE_PRS=$(gh pr list --search "${PR_TITLE}" --json number --jq ".[] | select(.number != ${NEW_PR_ID}) | .number")
